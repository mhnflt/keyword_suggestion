...
def test_cluster_search_results():
    ...
    assert any(
        c["domain"] == "example.com" and len(c["results"]) == 2 for c in clustered
    )
...
async def test_fetch_suggestions():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        with patch("main.get_keyword_volume", mock_get_keyword_volume), patch(
            "main.get_search_results", mock_get_search_results
        ):
